 ...
 */
public class OneTimeUserControlScreen extends Activity implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.layout_onetimeuser_control);
...
//        new TimerThread().run();
        ...
        new Timer().schedule(myTimer, 30000);
...
//                            Toast.makeText(getApplicationContext(), statusToast, Toast.LENGTH_LONG).show();
...
//                            Toast.makeText(getApplicationContext(), statusToast, Toast.LENGTH_LONG).show();
        ...
        protected Void doInBackground(String... status) {
            ...
            try {
                ...
                if (status[0] == "lock") {
                    send_message = "android:" + user_name + ":lock" + ":1";
                } else if (status[0] == "unlock") {
                    send_message = "android:" + user_name + ":unlock" + ":1";
                } else if (status[0] == "logout") {
                    send_message = "android:" + user_name + ":logout" + ":1";
                }
...
//                boolean changeScreen = false;
                while (true) {
                    ...
                    if (read.contains("loggedout")) {
                        Log.e("Server Sent  : " , "loggedout");
                        Intent viewIntent = new Intent(getApplicationContext(), HomeScreen.class);
                        startActivityForResult(viewIntent, 0);
//                        changeScreen = true;
                        break;
                    } else if (read.contains("android")) {
                        String statusText = null, statusToast = null;
...
//                            lock.setChecked(true);
                            ...
                            statusToast = "RaspberryPi is mismatched";
                            publishProgress(statusText, statusToast);
                        } else {
                            statusText = "UNKNOWN";
   ...
   /* public class TimerThread extends Thread {
        ...
        public void run() {
            ...
            try {
                this.wait(10000);
                new ClientThread().execute("logout");
                Intent viewIntent = new Intent(getApplicationContext(), HomeScreen.class);
                viewIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                startActivityForResult(viewIntent, 0);
    ...
    public class MyTimer extends TimerTask {
        ...
        @Override
        public void run() {
            Log.e("Android requesting : " , "logout");
            new ClientThread().execute("logout");
        }
    }
}