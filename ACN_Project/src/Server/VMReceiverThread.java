	...
	private Boolean validateOneTimeUser(String usr, String pwd) {
		...
		try {
			...
			while(rs.next()){
				...
				//				System.out.println("Start Time: "+rs.getDate(5));
				System.out.println("Flag value: "+rs.getBoolean("flag"));
				//				System.out.println("Duration: "+rs.getDouble(7));

				cnt = Integer.parseInt(rs.getString(1));
				//				startTime = rs.getDate(5);
			...
			if(cnt == 1){
				...
				if(!flag) {
					...
					//						rs = stmt.executeQuery("update OneTimeUser set STARTTIME = " + dateFormat.format(currentTime) + " where username='"+ usr +"' and password='"+ md5 +"';");
					//						
					//						
					//					} else {
					//						Date currentTime = new Date();
				...
				try {
					...
					if(words[0].equals("android_validation")){
						...
						if(words[2].equals("logout")) {
							//							if(words[3].equals("1")) {
							//								updateOneTimeUserAccess(words[1]);
							//							}
